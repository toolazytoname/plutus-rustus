 ...
 *
 * hash160(pubkey) = RIPEMD-160(SHA-256(pubkey)). Compressed pubkeys are one
 * SHA-256 block (33 bytes); uncompressed pubkeys are two (65 bytes). SHA-256
 * uses the ARMv8 crypto instructions; RIPEMD-160 is done 4 lanes at a time in
 * NEON. Compiled only on aarch64 (see build.rs).
 */
...
/* One SHA-256 compression of a 64-byte block into the running ABCD/EFGH state. */
static void sha256_compress(uint32x4_t *s0, uint32x4_t *s1, const uint8_t block[64]) {
...
static void sha256_init(uint32x4_t *s0, uint32x4_t *s1) {
...
static void sha256_store(uint32x4_t s0, uint32x4_t s1, uint8_t out[32]) {
    vst1q_u8(out + 0, vrev32q_u8(vreinterpretq_u8_u32(s0)));
...
static void sha256_33(const uint8_t *msg, uint8_t out[32]) {
    ...
    uint32x4_t s0, s1;
    sha256_init(&s0, &s1);
    sha256_compress(&s0, &s1, block);
    sha256_store(s0, s1, out);
}

/* SHA-256 of exactly 65 bytes (uncompressed pubkey) — two blocks. */
...
static void hash160_x4_uncomp(const uint8_t *pub /*4*65*/, uint8_t *out20 /*4*20*/) {
    uint8_t sha[4 * 32];
...
/* Same as hash160_many, but for 65-byte uncompressed pubkeys. */
